...
- **The client decides, the consultant informs.** Rate and prioritize
  findings objectively (via `references/compliance/risk-matrix.md`), but never make the
  sequencing decision for her — present the picture, ask what she wants next.

## Files here

- `references/ux/ux-heuristics.md` — Nielsen Norman Group’s 10 usability heuristics
  ...
  Sinaida’s own Perplexity research guide (`assets/research/perplexity_guide.pdf`).
  Read before content approval and before any audit.
- `references/compliance/security-checklist.md` — HTTPS/TLS, security headers, auth, input
  handling, backups. Non-negotiable baseline for every build and audit.
- `references/compliance/accessibility-wcag.md` — full WCAG 2.1 Level AA checklist
...
- `references/compliance/gdpr-checklist.md` — cookie banner behavior, privacy policy required
  fields, consent logging. Read before writing `templates/privacy-policy.md` or
  `templates/cookie-banner.html`.
- `references/design/anti-slop.md` — catalog of AI-generated-web-design tells (purple
...
- `references/design/animation-principles.md` — Disney’s Twelve Basic Principles of
...
- `references/design/motion-craft.md` — the concrete numbers that pair with the
  principles above: the “should this even animate” frequency gate, exact
  easing/duration tokens, spring configs, GPU-only property rules, and a
  literal never-ship table (Emil Kowalski’s animation philosophy + Apple’s
  WWDC fluid-interfaces guidance). Read alongside `animation-principles.md`
  every time — one gives the shape, this gives the tokens.
- `references/growth/seo-checklist.md` — technical SEO checklist curated from the
...
- `references/growth/agentic-readiness.md` — checklist distilled from
...
- `references/design/fonts.md` — pre-vetted commercial-free, high-readability font
  stacks. Pick from here; don’t introduce a new font without checking its license.
- `references/design/typography.md` — heading hierarchy, modular type scale, line-
...
- `references/ux/performance-adaptive.md` — how to build the fast/slow-network,
  low/high-GPU adaptive mode (a “light mode” that degrades gracefully).
- `references/process/gh-workflow.md` — issues-as-task-manager + branch hygiene for
  syncing work to `sinaida-space/aliens-built-my-website`.
- `references/design/design-resources.md` — curated external references (Aura,
...
- `references/design/glow-logo.md` — HDR glow/bloom logo technique. **Opt-in only** —
...
- `references/compliance/risk-matrix.md` — the Impact × Probability rating method every
...
- `templates/audit-prd.md` — the actual audit **deliverable** format: executive
  ...
  `references/compliance/risk-matrix.md`, a sortable risk-summary table, and a "what’s
...
7. **Bloom/glow branding?** — default is no; only proceed to `references/design/glow-logo.md`
...
`references/design/design-resources.md` matched to her stated priorities (question 6
...
Once a direction is picked, write it down as concrete values before touching
code — not “modern and clean” in chat (that expires when the session ends),
but a short token block at the top of `tokens.css`: color/type/spacing/radius/
shadow/animation values, **plus an explicit Forbidden list** naming the
`references/design/anti-slop.md` tells that apply to this specific brief (e.g. "no
purple gradients,“ ”no icon-in-a-box feature cards,“ ”no bounce easing").
Deciding what’s forbidden *before* implementation, not catching it at
self-review, is cheaper and the Forbidden list matters at least as much as
the color values next to it.
...
- [] A single `tokens.css` (or equivalent) holding every color, font, spacing
      unit, and grid value used on the site — pulled from `references/design/fonts.md`
      and `references/design/typography.md`. Every other file references these tokens;
...
- [] Full WCAG 2.1 AA pass per `references/compliance/accessibility-wcag.md` — semantic
...
- [] Fonts from `references/design/fonts.md` only.
- [] Typography per `references/design/typography.md` — real modular type scale
...
- [] Adaptive/light mode per `references/ux/performance-adaptive.md`.
- [] Security headers + HTTPS assumptions per `references/compliance/security-checklist.md`.
...
- [] SEO basics per `references/growth/seo-checklist.md` (semantic structure, meta tags,
      sitemap, robots.txt).
- [] Agentic-readiness quick wins per `references/growth/agentic-readiness.md`:
...
- [] `references/design/anti-slop.md` swept during implementation, not just at the end —
...
- [] Every animation (hover, transition, scroll-triggered reveal) built against
      `references/design/animation-principles.md` and `references/design/motion-craft.md`
      from the start — passes the "should this even animate" gate, eased not
      linear (exact tokens, never approximated), staggered not simultaneous,
      `transform`/`opacity` only, `prefers-reduced-motion` honored unconditionally.

### 4. Self-review before handing back
Re-run `references/ux/ux-heuristics.md` (including its cognitive-load and
persona pass), `references/compliance/accessibility-wcag.md`,
`references/compliance/security-checklist.md`, `references/design/anti-slop.md`,
`references/design/typography.md`, `references/design/animation-principles.md`, and
`references/design/motion-craft.md` (its Never Ship table specifically) as a
...
Close with a 60-second visual pass on the real, built pages — this is a fast
final sweep, not a replacement for the checklists above: text contrast holds
up on a phone screen in bright light, the layout survives at 375px width
(tables and long headlines break first), every interactive element shows a
visible hover *and* focus state, one radius/shadow/spacing system is used
consistently across every page (no drift back to an unforbidden default),
button copy names the actual outcome (“Book a 20-minute call,” not “Submit”),
and every image is optimized to tens of KB, not megabytes.

### 5. Ship via GitHub properly
Follow `references/process/gh-workflow.md`: an issue per feature/page, a branch per issue,
...
2. **Walk the checklist.** Use `templates/audit-checklist.md` against
   `references/ux/ux-heuristics.md` (including its cognitive-load scan and a
   2–3 persona pass on the primary conversion action),
   `references/compliance/accessibility-wcag.md` (the full
   WCAG 2.1 AA pass, not just a contrast/alt-text spot check),
   `references/compliance/security-checklist.md`, `references/compliance/gdpr-checklist.md`,
   `references/growth/seo-checklist.md`, `references/growth/agentic-readiness.md` (robots.txt
   ...
   `references/ux/performance-adaptive.md`,
   `references/design/fonts.md` (license + readability), `references/design/typography.md`
   (heading hierarchy, type scale, micro-typography), `references/design/anti-slop.md`
   (treat repeated hits across categories, not one stray gradient, as a real
   finding), and `references/design/animation-principles.md` +
   `references/design/motion-craft.md`'s Never Ship table for any hover/
...
3. **Turn every real gap into a numbered finding.** Sequential IDs (F-001,
   ...
   Probability, then combine via `references/compliance/risk-matrix.md` — never assign an
...
Even so, **only the checklist knowledge in `references/growth/seo-checklist.md` was ported**
